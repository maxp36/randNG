...
public class PearsonTest {
    ...
    private double[] sLevels = new double[]{.995, .990, .975, .950, .900, .750, .500, .250, .100, .050, .025, .010, .005};
    ...
    public double computeX2(List<Float> numbers, double theor) {
        double x2 = 0.0;
        for (Float n : numbers) {
            x2 += ((n - theor) * (n - theor)) / theor;
        }
        return x2;
    }

    public double computeX2(List<Double> numbers, List<Double> theor) {
        double x2 = 0.0;
        for (int i = 0; i < numbers.size(); i++) {
            x2 += ((numbers.get(i) - theor.get(i)) * (numbers.get(i) - theor.get(i))) / theor.get(i);
        }
        return x2;
    }

    public boolean compareX2(double x2, int df, double sLevel) {
        return x2 < getTheorX2(df, sLevel);
    }

    public double getTheorX2(int df, double sLevel) {
        int sl = 0;
        if (sLevel == sLevels[0]) {
            sl = 0;
        } else if (sLevel == sLevels[1]) {
            sl = 1;
        } else if (sLevel == sLevels[2]) {
            sl = 2;
        } else if (sLevel == sLevels[3]) {
            sl = 3;
        } else if (sLevel == sLevels[4]) {
            sl = 4;
        } else if (sLevel == sLevels[5]) {
            sl = 5;
        } else if (sLevel == sLevels[6]) {
            sl = 6;
        } else if (sLevel == sLevels[7]) {
            sl = 7;
        } else if (sLevel == sLevels[8]) {
            sl = 8;
        } else if (sLevel == sLevels[9]) {
            sl = 9;
        } else if (sLevel == sLevels[10]) {
            sl = 10;
        } else if (sLevel == sLevels[11]) {
            sl = 11;
        } else if (sLevel == sLevels[12]) {
            sl = 12;
        }

        return table[df - 1][sl];
    }
